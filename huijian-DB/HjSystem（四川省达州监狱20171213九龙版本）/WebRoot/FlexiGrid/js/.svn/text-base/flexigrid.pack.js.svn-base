(function($){
	$.addFlex=function(t,p){
		...
		p=$.extend({
			...
			qtype:'',
			nomsg:'No items',
			minColToggle:1,showToggleBtn:true,
			hideOnSubmit:true,
			autoload:true,
			blockOpacity:0.5,
			preProcess:false,
			...
			var g={
					...
					$('thead tr:first th:visible',this.hDiv).each(
							function(){
								...
								$('div:eq('+n+')',g.cDrag).css({'left':cdpos+'px'}).show();cdleft=cdpos
							}
					)
				},
		fixHeight:function(newH){
			...
			,dragStart:function(dragtype,e,obj){
						if(dragtype=='colresize'){
							$(g.nDiv).hide();
							$(g.nBtn).hide();var n=$('div',this.cDrag).index(obj);
...
//							this.dcol=obj;this.dcoln=$('th',this.hDiv).index(obj);
									...
									if(this.colresize){
										...
										if(this.dcolt!=null){
											if(this.dcoln>this.dcolt)
												...
												this.switchCol(this.dcoln,this.dcolt);$(this.cdropleft).remove();
												$(this.cdropright).remove();this.rePosDrag();
										...
										$(this.cDrag).show()}$('body').css('cursor','default');
										$('body').noSelect(false)}
											...
											$('tbody tr',t).each(function(){
												...
												this.hDiv.scrollLeft=this.bDiv.scrollLeft},scroll:function(){
													this.hDiv.scrollLeft=this.bDiv.scrollLeft;
													this.rePosDrag()},addData:function(data){
														if(p.dataType=='json'){
														...
														}else{p.total=data.total}if(p.total==0){
															$('tr, a, td, div',t).unbind();$(t).empty();
															p.pages=1;p.page=1;
															this.buildpager();
														...
														if(p.dataType=='json'){
															$.each(data.rows,function(i,row){
																...
																$('thead tr:first th',g.hDiv).each(function(){
																	...
																	var idx=$(this).attr('axis').substr(3);td.align=this.align;
															...
															var i=1;$("rows row",data).each(function(){
																i++;
																var tr=document.createElement('tr');
																...
																nid=null;
																var robj=this;$('thead tr:first th',g.hDiv).each(function(){
																	var td=document.createElement('td');
																	var idx=$(this).attr('axis').substr(3);
																	td.align=this.align;td.innerHTML=$("cell:eq("+idx+")",robj).text();
																	$(td).attr('abbr',$(this).attr('abbr'));
																	$(tr).append(td);td=null
																});
																if($('thead',this.gDiv).length<1){
															...
															$('tr',t).unbind();$(t).empty();$(t).append(tbody);
															this.addCellProp();this.addRowProp();
															this.rePosDrag();tbody=null;data=null;
															i=null;
															if(p.onSuccess){
																p.onSuccess(this)
															}
														...
														}else{this.populate()}},buildpager:function(){
															$('.pcontrol input',this.pDiv).val(p.page);
															$('.pcontrol span',this.pDiv).html(p.pages);
															var r1=(p.page-1)*p.rp+1;
															var r2=r1+p.rp-1;
															if(p.total<r2){r2=p.total}
															var stat=p.pagestat;stat=stat.replace(/{from}/,r1);
															stat=stat.replace(/{to}/,r2);
															stat=stat.replace(/{total}/,p.total);$('.pPageStat',this.pDiv).html(stat)
														},
														populate:function(){
															...
															this.loading=true;
															if(!p.url){return false}$('.pPageStat',this.pDiv).html(p.procmsg);
																	...
																	$(this).click(function(e){
																		...
																		if(p.singleSelect)
																			$(this).siblings().removeClass('trSelected')
																				...
																				if(e.shiftKey){$(this).toggleClass('trSelected');
																				g.multisel=true;this.focus();$(g.gDiv).noSelect()}
																			}).mouseup(function(){
