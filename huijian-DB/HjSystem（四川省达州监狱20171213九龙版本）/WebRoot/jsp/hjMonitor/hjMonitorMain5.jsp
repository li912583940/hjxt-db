...
<html xmlns="http://www.w3.org/1999/xhtml">
 <head>
		...
		function insertAddTime(){
			
			var lineCh=document.getElementById("lineCh1").value; 
		 	var serverNameCh=document.getElementById("serverNameCh").value;
		 	var monitorCallIDCh=document.getElementById("monitorCallIDCh").value;
		 	var monitorStateCh=document.getElementById("monitorStateCh").value;
		 	
		 	var time=document.getElementById("time");
			var time2='';
			for(var i=0;i<time.length;i++){
				if(time.options[i].selected){
					time2=time.options[i].value;
					break;
				}
			}
//			alert(lineCh);
//			alert(serverNameCh);
//			alert(monitorCallIDCh);
//			alert(monitorStateCh);
//			alert(time2);

   			if(monitorStateCh=="通话" && lineCh!="" && serverNameCh!="" && monitorCallIDCh!="" && monitorStateCh!=""){
   				document.getElementById("load").innerHTML="<img height =\"50px\" width=\"50px\" src=\"images/wait.gif\" /><b>处理中</b>";
				document.getElementById("load").style.display="block";
   				lineCh=encodeURI(encodeURI(lineCh));
   				serverNameCh=encodeURI(encodeURI(serverNameCh));
   				monitorCallIDCh=encodeURI(encodeURI(monitorCallIDCh));
   				time2=encodeURI(encodeURI(time2));
   				$.ajax({
   			      type:"POST",
   			      url:"hjMonitor.do",
   			      data:"method=insertAddTime&lineCh="+lineCh+"&serverNameCh="+serverNameCh+"&monitorCallIDCh="+monitorCallIDCh+"&time2="+time2,
   			      dataType:"json",
   			      success:callback2,
   			      error:callback1
   				});
   	   		}else{
   	   			alert("无法修改时长，当前线路已经没有在通话");
				return false;
   	   	   	}			
		}
		function callback2(data){
			if(data[0]=="0"){
				...
				document.getElementById("load").style.display="none";
				alert("修改通话时间失败，可能当前线路已经没有在通话或者账户权限不够");
				return false;
			}
		}
		function jiantingyinpin(){
			...
			var monitorCallIDs = document.getElementsByName("monitorCallID");
			var writeTxts = document.getElementsByName("writeTxt");
			var monitorStates = document.getElementsByName("monitorState");
			...
			var monitorCallID = monitorCallIDs[tr.rowIndex-1].value;
			var writeTxt = writeTxts[tr.rowIndex-1].value;
			var monitorState = monitorStates[tr.rowIndex-1].value;
			...
			if(monitorState=="通话"){
				if(document.getElementById("info").innerText==""){
					...
					tr.cells[8].children[1].innerHTML="<a href=\"javascript:void(0)\" onclick=\"jiesujianting();return false;\">停止监听</a>";
			  	    ...
			  	    document.getElementById("monitorCallIDCh").value=monitorCallID;
			  		document.getElementById("monitorStateCh").value=monitorState;
			  	    document.getElementById("ingHjid").value=hjid;
			  	   	document.getElementById("ingLineType").value=lineType;
			  	    document.getElementById("monitorCallID").value=monitorCallID;
			  	    document.getElementById("writeTxt").value=writeTxt;
					...
					for(var i=0;i<uptr.length;i++){
			          ...
			          if(uptd[1].innerText!=lineNo || uptd[0].innerText!=serverName){
		   	          	uptd[8].children[0].innerHTML="<span id=\"jt\">监听</span>";
...
//				 document.getElementById("addtime").style.display="block";
		    		...
		    		$.ajax({
		    		      type:"POST",
		    		      url:"hjMonitor.do",
		    		      data:"method=addSaveMonitorInfo&monitorCallID="+monitorCallID,
		    		      dataType:"json",
		    		      success:callback666,
		    		      error:callback1
		    		});
		...
		function jiesujianting(){
			...
			if(jttype=='0' || jttype==''){
				...
				document.getElementById("monitorCallIDCh").value="";
				document.getElementById("monitorStateCh").value="";
				...
				for(var i=0;i<uptr.length;i++){
			          ...
			          if(uptd[1].innerText!=lineNo || uptd[0].innerText!=serverName){
			                ...
			                uptd[8].children[0].innerHTML="<span><a href=\"javascript:void(0)\" onclick=\"jiantingyinpin();return false;\">监听</a></span>";
			          }
			    }
			 }else{
			 	var lineNos = document.getElementsByName("lineNo");
...
//			var monitorState = monitorStates[tr.rowIndex-1].value;	
 			...
 			if(info==""){
    			alert("没有在监听中");
     			...
     			var monitorCallID=document.getElementById("monitorCallID").value;
        		...
        		if(monitorCallID==""){
           			alert("该线路未处于通话中，无法录入摘要");
       			 }else{
         			if(ingmonitorFlagId==""){

	         			 document.getElementById("info1").innerHTML="摘要内容：<input type=\"text\" name=\"ingwriteTxt\" id=\"ingwriteTxt\" value=\""+writeTxt+"\">&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"保存\" onclick=\"addSaveMonitorFlag();\"><input type=\"hidden\" id=\"ingmonitorCallID\" name=\"ingmonitorCallID\" value=\""+monitorCallID+"\"></a>";
	       			}else{

	        			 document.getElementById("info1").innerHTML="摘要内容：<input type=\"text\" name=\"ingwriteTxt\" id=\"ingwriteTxt\" value=\""+writeTxt+"\">&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"保存\" onclick=\"updateSaveMonitorFlag();\" /><input type=\"hidden\" id=\"ingmonitorFlagId\" name=\"ingmonitorFlagId\" value=\""+ingmonitorFlagId+"\"></a>";
	        		}
     			 }
 			} 		
		 }
	  ...
	  <div id="lm_info5"><span>当前位置：实时监控</span></div>
	       <form action="" method="post">
		 			...
		 			<logic:notEmpty name="sysQqLineList">
		 				<tbody id="info5">
					     	   ...
					     	   <tr>
					     	       ...
					     	       <td width="14%" nowrap="nowrap"><span id="jtyp"><a href="javascript:void(0)" onclick="jiantingyinpin();return false;">监听</a></span>
					     	                                      ...
					     	                                      <input type="hidden" name="monitorState" value="${sl.monitorState }" />
					     	      </td>
					     	   </tr>
						  	</logic:iterate>
		   	...
		   	<div id="info6" style="position:absolute;margin-top:110px;margin-left:180px;height:25px; line-height:25px;"><font color="green">三楼</font>&nbsp;&nbsp;&nbsp;&nbsp;会见窗口使用情况：总数量&nbsp;&nbsp;<font color="red">27</font>&nbsp;&nbsp; 个，正在使用&nbsp;&nbsp;<font color="red">${hjm.fourShiyong }</font>&nbsp;&nbsp;个，未使用&nbsp;&nbsp;<font color="red">${hjm.fourWeishiyong }</font>&nbsp;&nbsp;个，屏蔽&nbsp;&nbsp;<font color="red">${hjm.fourPingbi }</font>&nbsp;&nbsp;个；</div>-->
		   		 <div id="info2"></div><input type="hidden" id="jtyinpin1" vaule="0" /><input type="hidden" id="monitorCallID"  /><input type="hidden" id="writeTxt"  />
		   		 <div id="load" style="display:none;position:absolute;margin-top:1%;position:absolute;z-index:2;margin-left:70%"></div>
		   </form>
