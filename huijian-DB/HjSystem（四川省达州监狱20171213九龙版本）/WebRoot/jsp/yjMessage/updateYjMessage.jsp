	  ...
	  <div id="lm_info"><span>当前位置：警察信息</span></div>
	       <form action="yjMessage.do?method=search" method="post">
			     	<table  style="border-style:solid; border-color:#76a5af; border-width:1px 0 0 1px; width:60%; margin:0 auto; margin-top:10px;">
			     		<tr>
			     			<td width="30%">警察编号：</td>
			     			<td width="30%"><input type="text" id="yjNum1" name="yjNum1" value="${jlYj.yjNum }" disabled="disabled" /></td>
			     		</tr>
			     		<tr>
			     			<td>警察名称：</td>
			     			<td><input type="text" id="yjName1" name="yjName1" value="${jlYj.yjName }" /></td>
			     		</tr>
			     		<tr>
			     			<td>警察IC卡：</td>
			     			<td><input type="text" id="yjCard" name="yjCard" value="${jlYj.yjCard }" /></td>
			     		</tr>
			     		<tr>
			     			<td >部门：</td>
			     			<td >
		     				<select id="deptName">
		     					<logic:iterate id="jl" name="deptList">
		     						<c:if test="${jlYj.deptName==jl}"><option value="${jl}" selected="selected">${jl}</option></c:if>
			     		...
			     		<!-- <tr>
			     			<td>服务器名称：</td>
			     			<td>
			     				<select id="jyId" style="width:130px;" onchange="checkJq1();">
			     						<logic:iterate id="sqs" name="sysQqServerList">
			      	 					<c:if test="${sqs.serverName == jlYj.jy}">
			     		...
			     		<tr>
			     			<td>所属监区：</td>
			     			<td>
			     				<select id="jqId" style="width:130px;">
			     						<logic:iterate id="jq" name="jlJqList">
			     						       <c:if test="${jq.jqNo==jlYj.jq}"><option value="${jq.jqNo}" selected>${jq.jqName}</option></c:if>
			     							   <c:if test="${jq.jqNo!=jlYj.jq}"><option value="${jq.jqNo}">${jq.jqName}</option></c:if>
			     						</logic:iterate>
			     		...
			     		<tr>
			     			<td><a href="javascript:void(0);" onclick="saveUpdateYj();return false;"><img src="images/baocun.gif"></img><input type="hidden" id="webId" name="webId" value="${jlYj.webId}" /></td>
